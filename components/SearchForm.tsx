...
export default function SearchForm({ onSearch, isLoading }: { onSearch: (url: string, honeypot?: string) => void, isLoading?: boolean }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full max-w-2xl mx-auto flex items-center bg-zinc-900/40 border border-zinc-800 rounded-xl overflow-hidden focus-within:border-white/50 transition-colors">
      ...
      <button 
        ...
        className="bg-white hover:bg-zinc-200 text-black font-bold px-8 py-4 transition-all tracking-tight text-lg cursor-pointer disabled:opacity-70 disabled:cursor-not-allowed flex items-center gap-2"
      >
        {isLoading ? (
          <>
